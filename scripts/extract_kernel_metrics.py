#!/usr/bin/env python3
"""
Script to extract individual kernel metrics from NVIDIA profiler log files.
Takes a log file and a target metric as input, extracts the metric values 
for each individual kernel and outputs to a CSV file.

Usage: python extract_kernel_metrics.py <log_file> <metric_name> [output_file]
"""

import sys
import re
import csv
from typing import List, Dict

def parse_log_file_individual(log_file_path: str, target_metric: str) -> List[Dict]:
    """
    Parse the log file and extract the target metric for each individual kernel.
    
    Args:
        log_file_path: Path to the log file
        target_metric: Name of the metric to extract (e.g., 'gpu__time_duration.sum')
    
    Returns:
        List of dictionaries containing individual kernel data
    """
    kernel_data = []
    current_range = None
    current_kernel = None
    in_kernel_section = False
    kernel_counter = 0
    
    with open(log_file_path, 'r') as file:
        for line_num, line in enumerate(file, 1):
            line = line.strip()
            
            # Check for range name
            if line.startswith("Range Name:"):
                current_range = line.replace("Range Name:", "").strip()
                continue
            
            # Check for kernel line (starts with "Kernel:")
            if line.startswith("Kernel:"):
                current_kernel = line.replace("Kernel:", "").strip()
                kernel_counter += 1
                continue
            
            # Check for separator lines
            if line.startswith("======================================================================================"):
                in_kernel_section = False
                current_kernel = None
                continue
            
            if line.startswith("-----------------------------------------------------------------------------------"):
                if current_kernel:
                    # This marks the start of metrics for the current kernel
                    in_kernel_section = True
                else:
                    # This marks the end of metrics for the current kernel
                    in_kernel_section = False
                continue
            
            # Parse metric lines when in kernel section
            if in_kernel_section and current_range and current_kernel:
                # Look for lines with metric values (format: metric_name    value)
                if re.match(r'^[a-zA-Z_][a-zA-Z0-9_.]*\s+[\d.]+$', line):
                    parts = line.split()
                    if len(parts) >= 2:
                        metric_name = parts[0]
                        metric_value = parts[-1]
                        
                        # Check if this is the target metric
                        if metric_name == target_metric:
                            # Truncate kernel name for better readability
                            kernel_display = current_kernel
                            if len(kernel_display) > 150:
                                kernel_display = kernel_display[:150] + "..."
                            
                            kernel_data.append({
                                'kernel_id': kernel_counter,
                                'range_name': current_range,
                                'kernel_name': kernel_display,
                                'metric_name': metric_name,
                                'metric_value': float(metric_value),
                                'line_number': line_num
                            })
    
    return kernel_data

def write_individual_csv(data: List[Dict], output_file: str, target_metric: str):
    """
    Write the individual kernel data to a CSV file.
    
    Args:
        data: List of dictionaries containing the extracted data
        output_file: Path to the output CSV file
        target_metric: Name of the target metric
    """
    if not data:
        print(f"No data found for metric '{target_metric}'")
        return
    
    # Write CSV file
    with open(output_file, 'w', newline='', encoding='utf-8') as csvfile:
        fieldnames = ['kernel_id', 'range_name', 'kernel_name', target_metric]
        
        writer = csv.DictWriter(csvfile, fieldnames=fieldnames)
        writer.writeheader()
        
        # Write data for each kernel
        for kernel in data:
            writer.writerow({
                'kernel_id': kernel['kernel_id'],
                'range_name': kernel['range_name'],
                'kernel_name': kernel['kernel_name'],
                target_metric: f"{kernel['metric_value']:.3f}"
            })
    
    print(f"Individual kernel data successfully written to {output_file}")
    print(f"Total kernels with '{target_metric}': {len(data)}")
    
    # Print summary by range
    range_summary = {}
    for kernel in data:
        range_name = kernel['range_name']
        if range_name not in range_summary:
            range_summary[range_name] = []
        range_summary[range_name].append(kernel['metric_value'])
    
    print("\nSummary by range:")
    print(f"{'Range Name':<25} {'Count':<8} {'Min':<12} {'Max':<12} {'Avg':<12}")
    print("-" * 75)
    for range_name, values in range_summary.items():
        count = len(values)
        min_val = min(values)
        max_val = max(values)
        avg_val = sum(values) / count
        print(f"{range_name:<25} {count:<8} {min_val:<12.3f} {max_val:<12.3f} {avg_val:<12.3f}")

def main():
    """Main function to handle command line arguments and orchestrate the extraction."""
    if len(sys.argv) < 3:
        print("Usage: python extract_kernel_metrics.py <log_file> <metric_name> [output_file]")
        print("\nThis script extracts individual kernel metrics (not accumulated by range).")
        print("\nExample metrics from the log file:")
        print("  - gpu__time_duration.sum")
        print("  - gpu__time_duration.max")
        print("  - gpc__cycles_elapsed.avg.per_second")
        print("  - gpc__cycles_elapsed.max")
        print("  - sm__cycles_active.max")
        sys.exit(1)
    
    log_file = sys.argv[1]
    target_metric = sys.argv[2]
    output_file = sys.argv[3] if len(sys.argv) > 3 else f"individual_{target_metric.replace('.', '_')}.csv"
    
    print(f"Extracting individual kernel values for metric '{target_metric}' from '{log_file}'...")
    
    try:
        # Extract data
        data = parse_log_file_individual(log_file, target_metric)
        
        if not data:
            print(f"No instances of metric '{target_metric}' found in the log file.")
            print("Please check the metric name and ensure it exists in the log file.")
            return
        
        # Write to CSV
        write_individual_csv(data, output_file, target_metric)
        
    except FileNotFoundError:
        print(f"Error: File '{log_file}' not found.")
        sys.exit(1)
    except Exception as e:
        print(f"Error processing file: {str(e)}")
        sys.exit(1)

if __name__ == "__main__":
    main()