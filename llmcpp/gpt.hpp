...
int curr_fwd_block = 0;
int curr_bwd_block = 0;
...
namespace gpt
{
  ...
  struct Block
  {
                 ...
                 typename TTypes<Type, 3>::Tensor y)
    {
      ...
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->pushRange("LN1_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("LN1", ln1_->Forward(x_2d, ln1_y_2d, ln1_mean_1d, ln1_rstd_1d));
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->popRange("LN1_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      // Attention
...
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->pushRange("Attention_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("Attention", attn_->Forward(ln1_y_3d, att_y_3d));
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->popRange("Attention_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      // Residual
...
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->pushRange("Residual_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("Residual", nn::Residual::Forward(x_1d, att_y_1d, residual1_1d));
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->popRange("Residual_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      // LN2
...
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->pushRange("LN2_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("LN2", ln2_->Forward(residual1_2d, ln2_y_2d, ln2_mean_1d, ln2_rstd_1d));
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->popRange("LN2_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      // MLP
      auto mlp_y_2d = MakeMatrix(mlp_y_->data<Type>(), B * T, C);
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->pushRange("MLP_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("MLP", mlp_->Forward(ln2_y_2d_const, mlp_y_2d));
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->popRange("MLP_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      // Residual
...
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->pushRange("Residual2_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("Residual2", nn::Residual::Forward(residual1_1d_const, mlp_y_1d, y_1d));
#ifdef TRAINING_FORWARD
      if(curr_step == 1 && curr_fwd_block == 1)
        GmpProfiler::getInstance()->popRange("Residual2_" + std::to_string(curr_fwd_block), GmpProfileType::CONCURRENT_KERNEL);
#endif
    }
                  ...
                  typename TTypes<Type, 3>::Tensor x_grad)
    {
      ...
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->pushRange("Residual2_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("Residual2_Backward", {nn::Residual::Backward(y_grad_1d, residual1_grad_1d, mlp_y_grad_1d);});
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->popRange("Residual2_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
...
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->pushRange("MLP_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("MLP_Backward", {mlp_->Backward(ln2_y_2d, mlp_y_grad_2d, ln2_y_grad_2d);});
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->popRange("MLP_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
...
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->pushRange("LN2_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("LN2_Backward", {ln2_->Backward(residual1_2d, ln2_y_grad_2d_const, ln2_mean_1d, ln2_rstd_1d,
                     residual1_grad_2d);});
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->popRange("LN2_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
      // backward residual
...
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->pushRange("Residual1_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("Residual1_Backward", {nn::Residual::Backward(residual1_grad_1d_const, x_grad_1d, att_y_grad_1d);});
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->popRange("Residual1_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
...
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->pushRange("Attention_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("Attention_Backward", {attn_->Backward(ln1_y_3d, att_y_grad_3d, ln1_y_grad_3d);});
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->popRange("Attention_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
...
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->pushRange("LN1_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("LN1_Backward", {ln1_->Backward(x_2d, ln1_y_grad_2d, ln1_mean_1d, ln1_rstd_1d, x_grad_2d);});
#ifdef TRAINING_BACKWARD
      if(curr_bwd_block == 1 && curr_step == 1) GmpProfiler::getInstance()->popRange("LN1_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
    }
  ...
  struct GPT
  {
    ...
    void __Forward(typename TTypes<int>::ConstMatrix idx)
    {
      ...
      #ifdef TRAINING_FORWARD
      if(curr_step == 1)GmpProfiler::getInstance()->pushRange("LayerNorm_Final", GmpProfileType::CONCURRENT_KERNEL);
      #endif
      GMP_TIMED("LayerNorm_Final", lnf_->Forward(block_out_2d, lnf_y, lnf_mean, lnf_rstd));
      #ifdef TRAINING_FORWARD
      if(curr_step == 1)GmpProfiler::getInstance()->popRange("LayerNorm_Final", GmpProfileType::CONCURRENT_KERNEL);
      #endif
    }
                    ...
                    typename TTypes<Type, 3>::Tensor logits, float *loss)
    {
      ...
      #ifdef TRAINING_FORWARD
      if(curr_step == 1)GmpProfiler::getInstance()->pushRange("LM_Head_Forward", GmpProfileType::CONCURRENT_KERNEL);
      #endif
      GMP_TIMED("LM_Head_Forward", logits_2d.device(nn::g_device) = lnf_y.contract(lm_head, product_dims););
      #ifdef TRAINING_FORWARD
      if(curr_step == 1)GmpProfiler::getInstance()->popRange("LM_Head_Forward", GmpProfileType::CONCURRENT_KERNEL);
      #endif
    ...
    void BackwardGPU(typename TTypes<int>::ConstMatrix idx)
    {
...
#ifdef TRAINING_BACKWARD
      if(curr_step == 1) GmpProfiler::getInstance()->pushRange("LayerNorm_Final_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
      GMP_TIMED("LayerNorm_Final_Backward", lnf_->Backward(block_out_2d, lnf_y_grad_2d, lnf_mean, lnf_rstd,
                     block_out_grad_2d););
#ifdef TRAINING_BACKWARD
       if(curr_step == 1) GmpProfiler::getInstance()->popRange("LayerNorm_Final_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif      
      ...
      for (int l = n_layer_ - 1; l >= 0; --l)
      {
        ...
        if(l == n_layer_-2){
          GMP_TIMED("Block_Backward", block->Backward(block_x_3d, block_y_grad_3d, block_x_grad_3d));
        }
        else{
...
#ifdef TRAINING_BACKWARD
      if(curr_step == 1) GmpProfiler::getInstance()->pushRange("Tok_Pos_Embedding_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
GMP_TIMED("Tok_Pos_Embedding_Backward",
      {
...
#ifdef TRAINING_BACKWARD
      if(curr_step == 1) GmpProfiler::getInstance()->popRange("Tok_Pos_Embedding_Backward", GmpProfileType::CONCURRENT_KERNEL);
#endif
    }
